    ...
    <div className="min-h-screen bg-[#f5f1e6] flex items-center justify-center p-4">
      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          ...
          <p className="mt-3 text-[#7d6b56]">{isSignUp ? "Create your account" : "Welcome back"}</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-xl border border-[#e2d8c3] p-8">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-12 bg-gradient-to-r from-[#a67c52] to-[#8d6e4c] hover:from-[#8d6e4c] hover:to-[#735a3a] text-white font-medium shadow-lg">
              {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>{isSignUp ? "Create Account" : "Sign In"}<ArrowRight className="w-4 h-4 ml-2" /></>}
            </Button>
          </form>

          <p className="mt-6 text-center text-sm text-[#7d6b56]">
            {isSignUp ? "Already have an account?" : "Don't have an account?"}{" "}
            <button onClick={() => { setIsSignUp(!isSignUp); setMessage(null) }} className="text-[#a67c52] hover:text-[#8d6e4c] font-medium transition-colors">
        ...
        <p className="mt-6 text-center">
          <Link href="/" className="text-sm text-[#8a7e72] hover:text-[#a67c52] transition-colors">← Back to home</Link>
        </p>
      </div>
    </div>
  )
